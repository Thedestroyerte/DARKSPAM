...
from AltronX.data import ABUSE, RAID, REPLYRAID, ALTRON, MRAID, SRAID, CRAID, ALTRON
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(mkraid) == 2:
            ...
            g = a.id
            if int(g) in ALTRON:
                await e.reply("» ᴀʀᴇ.. ʏᴇ ᴛᴏ ᴛᴇʀᴀ  ᴅᴀʀᴋ ᴘᴀᴘᴀ ʜᴀɪ", parse_mode=None, link_preview=None)
            elif int(g) in SUDO_USERS:
                await e.reply("» ᴀʙᴇ.. ɪs ᴄʜᴜᴛɪʏᴀ ᴋᴇ ᴘᴀᴀs ᴘᴇʜʟᴇ sᴇ sᴜᴅᴏ ʜᴀɪ", parse_mode=None, link_preview=None)
            else:
                c = a.first_name
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in ALTRON:
                ...
                await e.reply("» ᴀʙᴇ.. ɪs ᴄʜᴜᴛɪʏᴀ ᴋᴇ ᴘᴀᴀs ᴘᴇʜʟᴇ sᴇ sᴜᴅᴏ", parse_mode=None, link_preview=None)
            else:
                que[user_id] = []
                gey = que.get(user_id)
                phucker = [user_id]
