...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"»⚡️4𝗦𝗧 𝗗𝗔𝗥𝗞⚡️_ᴏᴘ_ʙᴏʟᴛᴀ", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"_⚡️𝗔𝗕 𝗧𝗨 𝗧𝗢𝗛 𝗚𝗔𝗬𝗔 𝗖𝗛𝗔𝗠𝗔𝗥 𝗧𝗨 𝗖𝗛𝗨𝗗𝗡𝗘 𝗪𝗔𝗟𝗔 𝗛𝗔𝗜⚡️_\n» `{mp} ms`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"ʜᴀᴛᴛ ʙsᴅᴋ ᴛʜᴏᴅɪ ᴀᴜʀ ɢᴀᴀɴᴅ ᴍᴀʀɴᴇ ᴅᴇᴛᴀ...!😪😒")
        try:
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        if len(sudousers) > 0:
            ...
            newsudo = f"{target}"
        await ok.edit(f"» **4sᴛ ᴅᴀʀᴋ__ᴇᴋ ɴᴀʏᴀ ʙᴇᴛᴀ ᴀᴅᴅ ʜᴏʀᴀ...__**: `{target}`\n» `ʙsᴅᴋ ʀᴇʙᴏᴏᴛ ᴛᴏʜ ʜᴏɴᴇᴇ ᴅᴇ😁...`")
